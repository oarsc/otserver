...
void Game::proceduralRefresh(Map::TileMap::iterator* begin)
{
	...
	g_scheduler.addEvent(createSchedulerTask(100,
		std::bind(&Game::proceduralRefresh, this, begin)));
}
	...
	const Position& movingCreatureOrigPos, const Position& toPos)
{
	...
	if(!player->canDoAction()){
		...
		SchedulerTask* task = createSchedulerTask(delay, std::bind(&Game::playerMoveCreature, this, playerId, movingCreatureId,
	...
	if(!Position::areInRange<1,1,0>(movingCreatureOrigPos, player->getPosition())){
		...
		if(getPathToEx(player, movingCreatureOrigPos, listDir, 0, 1, true, true)){
			...
			SchedulerTask* task = createSchedulerTask(g_config.getNumber(ConfigManager::PUSH_INTERVAL), std::bind(&Game::playerMoveCreature, this,
				playerId, movingCreatureId, movingCreatureOrigPos, toPos));
			g_dispatcher.addTask(createTask(std::bind(&Game::playerRegisterWalkAction,
	...
	uint16_t spriteId, uint8_t fromStackPos, const Position& toPos, uint16_t count)
{
	...
	if(!Position::areInRange<1,1,0>(playerPos, mapFromPos)){
		...
		if(getPathToEx(player, item->getPosition(), listDir, 0, 1, true, true)){
			...
			SchedulerTask* task = createSchedulerTask(50, std::bind(&Game::playerMoveItem, this,
...
{
	remainderCount = (uint32_t) item->getItemCount();
	...
	if(!test){
		...
		if (remainderCount > 0) {
			...
			if((ret = internalAddItem(origToCylinder, remainderItem, INDEX_WHEREEVER, flags, false, remainderCount)) != RET_NOERROR){
				item->setItemCount(remainderCount);
	...
	if(ret != RET_NOERROR){
		if(ret == RET_TOOFARAWAY){
			...
			if(getPathToEx(player, walkToPos, listDir, 0, 1, true, true, 10)){
				...
				SchedulerTask* task = createSchedulerTask(player->getNextActionTime(), std::bind(&Game::playerUseItemEx, this,
	...
	if(ret != RET_NOERROR){
		if(ret == RET_TOOFARAWAY){
			...
			if(getPathToEx(player, pos, listDir, 0, 1, true, true, 10)){
				...
				SchedulerTask* task = createSchedulerTask(player->getNextActionTime(), std::bind(&Game::playerUseItem, this,
	...
	if(ret != RET_NOERROR){
		if(ret == RET_TOOFARAWAY){
			...
			if(getPathToEx(player, item->getPosition(), listDir, 0, 1, true, true)){
				...
				SchedulerTask* task = createSchedulerTask(player->getNextActionTime(), std::bind(&Game::playerUseBattleWindow, this,
...
bool Game::playerRotateItem(uint32_t playerId, const Position& pos, uint8_t stackPos, const uint16_t spriteId)
{
	...
	if(pos.x != 0xFFFF && !Position::areInRange<1,1,0>(pos, player->getPosition())){
		...
		if(getPathToEx(player, pos, listDir, 0, 1, true, true)){
			...
			SchedulerTask* task = createSchedulerTask(50, std::bind(&Game::playerRotateItem, this,
...
void Game::checkCreatureAttack(uint32_t creatureId)
{
	Creature* creature = getCreatureByID(creatureId);
	if(creature && creature->getHealth() > 0){
		if (creature->onAttacking(0)) {
			updateCreatureCheckIndex(creature->getPlayer());
		}
	}
}
...
void Game::addCreatureCheck(Creature* creature)
{
	...
	creature->checkCreatureVectorIndex = random_range(0, EVENT_CREATURE_COUNT - 1);
	creature->useThing2();
}

void Game::updateCreatureCheckIndex(Creature* creature)
{
	if(creature == NULL || creature->isRemoved()){
		return;
	}

	int32_t checkCreatureCurrentIndex = checkCreatureLastIndex+1;

	if(checkCreatureCurrentIndex >= EVENT_CREATURE_COUNT){
		checkCreatureCurrentIndex = 0;
	}

	if (creature->checkCreatureVectorIndex != checkCreatureCurrentIndex) {
		creature->checkCreatureVectorIndex = checkCreatureCurrentIndex;
		toAddCheckCreatureVector.push_back(creature);
	}
...
void Game::checkCreatures()
{
	...
	if(checkCreatureLastIndex == EVENT_CREATURE_COUNT){
		checkCreatureLastIndex = 0;
	}
