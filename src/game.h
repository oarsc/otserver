...
public:
  Game();
	...
	void updateCreatureCheckIndex(Creature* creature);
	...
	bool playerMoveThing(uint32_t playerId, const Position& fromPos, uint16_t spriteId, uint8_t fromStackPos, const Position& toPos, uint16_t count);
	...
	bool playerUseBattleWindow(uint32_t playerId, const Position& fromPos,
		uint8_t fromStackPos, uint32_t creatureId, uint16_t spriteId);
	...
	bool playerSetFightModes(uint32_t playerId, fightMode_t fightMode, chaseMode_t chaseMode, pickUpMode_t pickUpMode, bool safeMode);
...
protected:
	...
	std::vector<Creature*> checkCreatureVectors[EVENT_CREATURE_COUNT];
