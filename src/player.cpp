...
int32_t Player::getWeaponSkill(const Item* item) const
{
	...
	switch(weaponType){
		case WEAPON_PIERCE:
			attackSkill = getSkillLevel(SKILL_PIERCE);
			break;

		case WEAPON_STRIKE:
		{
			attackSkill = getSkillLevel(SKILL_STRIKE);
			break;
		}
...
std::string Player::getSkillName(int skillid)
{
	...
	case SKILL_FIST:
		...
		break;
	case SKILL_STRIKE:
		skillname = "strike fighting";
		break;
	case SKILL_PIERCE:
		skillname = "pierce fighting";
		break;
	case SKILL_SLASH:
...
bool Player::onAttacking(uint32_t interval)
{
	return Creature::onAttacking(interval);
}
